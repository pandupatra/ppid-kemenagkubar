...
export type DocumentReport = { id: string; title: string; href: string | null }
export type AdminDocumentReport = DocumentReport & {
...
async function ensureBucket(bucketId: string, isPublic: boolean) {
  ...
  const buckets = await result.json().catch(() => null)
  if (!result.ok || !Array.isArray(buckets))
...
function validateFile(data: FormData) {
  const title = String(data.get('title') ?? '').trim()
  const file = data.get('file')
  if (!title || !(file instanceof File) || !file.size)
    throw new Error('Judul dan berkas PDF wajib diisi.')
  if (file.size > maximumFileBytes || !file.name.toLowerCase().endsWith('.pdf'))
    throw new Error('Pilih PDF dengan ukuran maksimum 15 MB.')
  return { title, file }
}

export const getPublicDocumentReports = createServerFn({
  method: 'GET',
}).handler(async (): Promise<DocumentReport[]> => {
  const baseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL?.replace(/\/$/, '')
  const rows = await queryRows<{
    ...
    `select report.id, report.title, version.storage_path as "storagePath"
     from ppid.document_report_documents report
     join ppid.documents document on document.id = report.document_id and document.publication_state = 'published'
     join ppid.document_versions version on version.id = document.active_version_id
     order by report.display_order`,
  )
  return rows.map((row) => ({
    id: row.id,
    title: row.title,
...
export const getAdminDocumentReports = createServerFn({
  method: 'GET',
}).handler(async () => {
  const actor = await getAdminActor()
  if (!actor || !canReadContent(actor)) return { access: 'denied' as const }
  const documents = await queryRows<AdminDocumentReport>(
    `select report.id, report.title, version.original_filename as "attachmentName", document.publication_state as "publicationState",
      case when document.publication_state = 'published' then version.storage_path else null end as href
     from ppid.document_report_documents report
     left join ppid.documents document on document.id = report.document_id
     left join lateral (select original_filename, storage_path from ppid.document_versions where document_id = document.id order by created_at desc limit 1) version on true
     order by report.display_order`,
  )
  return {
...
export const saveDocumentReport = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const report = await queryRows<{ id: string }>(
      'select id from ppid.document_report_documents where id = $1',
      [data.id],
    )
    if (!report[0]) throw new Error('Dokumen atau laporan tidak ditemukan.')
    const documentId = randomUUID(),
      versionId = randomUUID(),
      storagePath = `document-reports/${documentId}/${versionId}.pdf`
    await ensureBucket(quarantineBucket, false)
    const upload = await storage(`/object/${quarantineBucket}/${storagePath}`, {
      method: 'POST',
    ...
    try {
      await withTransaction(async (client) => {
        ...
        await client.query(
          'update ppid.document_report_documents set title = $1, document_id = $2, updated_at = now() where id = $3',
          [data.title, documentId, data.id],
        )
        await client.query(
          `insert into ppid.audit_logs (actor_user_id, action, resource_type, resource_id, change_summary) values ($1, 'document_report.replaced', 'document_report', $2, $3::jsonb)`,
          [actor.userId, data.id, JSON.stringify({ versionId })],
        )
      })
    } catch (error) {
      await storage(`/object/${quarantineBucket}/${storagePath}`, {
        method: 'DELETE',
      }).catch(() => undefined)
      throw error
    }
...
export const publishDocumentReport = createServerFn({ method: 'POST' })
  .validator((data: unknown) => {
  ...
  .handler(async ({ data }) => {
    ...
    const rows = await queryRows<{
     ...
     from ppid.document_report_documents report join ppid.documents document on document.id = report.document_id
     join ppid.document_versions version on version.document_id = document.id
     where report.id = $1 and version.storage_bucket = $2 order by version.created_at desc limit 1`,
    ...
    const copied = await storage('/object/copy', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
    ...
    await withTransaction(async (client) => {
      ...
      await client.query(
        `insert into ppid.audit_logs (actor_user_id, action, resource_type, resource_id, change_summary) values ($1, 'document_report.published', 'document_report', $2, $3::jsonb)`,
